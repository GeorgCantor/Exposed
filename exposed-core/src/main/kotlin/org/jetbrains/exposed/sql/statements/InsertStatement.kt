...
open class InsertStatement<Key : Any>(
    ...
    private fun processResults(rs: ResultSet?, inserted: Int): List<ResultRow> {
        ...
        return arguments!!
            ...
            .map { unwrapped(extraDefaultAndNullValues(it)) + it }
            .map { ResultRow.createAndFillValues(it as Map<Expression<*>, Any?>) }
    }

    private fun unwrapped(values: Map<Column<*>, Any?>): Map<Column<*>, Any?> {
        return values.mapValues { (col, value) ->
            value?.let { (col.columnType as? ColumnWithTransform<Any, Any>)?.unwrapRecursive(it) } ?: value
        }
    }

    private fun extraDefaultAndNullValues(values: Map<Column<*>, Any?>): Map<Column<*>, Any?> {
